...
function check_income_data($arr){

	foreach($arr as $val){
		if (!isset($val) || (!$val) ) return false;
		htmlspecialchars(substr($val,0,32));
	}
	return true;
}


function check_PinT_id($PinT_id){
	$sql = "SELECT * FROM pin_types WHERE PinT_id = '$PinT_id'";
	$rez = mysql_query($sql) OR die(mysql_error());
	if(! (mysql_num_rows($rez)) ) return(array(0,0));
	$row = mysql_fetch_row($rez);
	return array($row[1],$row[2]);

}
// функция, которая возвращает  количество товаров указанного типа($PinT_id)

function count_pin($PinT_id){
	
	$sql = "SELECT count(Pin_id) FROM pin_list WHERE PinT_id = '$PinT_id' AND used=0";
	$rez = mysql_query($sql) OR die(mysql_error());
	$row = mysql_fetch_row($rez);
	return $row[0];
}
