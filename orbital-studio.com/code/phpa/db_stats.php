<?php
/* $Id: db_stats.php,v 1.32 2002/02/10 09:24:47 loic1 Exp $ */
...
if ($num_dbs > 0) {
    ...
    $common_url     = 'db_stats.php?lang=' . $lang . '&amp;server=' . $server;
    if (empty($sort_by)) {
        $sort_by                 = 'db_name';
        $sort_order              = 'asc';
    }
...
<form action="db_stats.php">
    ...
    while (list($db_name, $db_prop) = each($dbs_array)) {
        ...
        echo '        </td>' . "\n";
        echo '        <td bgcolor="'. $bgcolor . '">&nbsp;<a href="index.php?lang=' . $lang . '&amp;server=' . $server . '&amp;db=' . urlencode($db_name) . '" target="_parent">' . htmlentities($db_name) . '</a>&nbsp;</td>' . "\n";
