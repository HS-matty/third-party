...
//print_r($items);exit();
	...
	public static function prepareComment( &$comment )
	{
		...
		if ($acl->check('enable_autocensor')) {
			...
			if ($comment->position != '') {
				$comment->position = JCommentsText::censor($comment->position);
			}
		}
    ...
    public static function getCategoryComments( $object_id, $object_group = 'com_content', $page = 0 )
    {
        $object_id = (int)$object_id;
        $object_group = trim( $object_group );
        ...
        //number of comments per one article
        $limitend=3;
...
$lim = 0;
            ...
            foreach ($rows as $row) {
                ...
                // setup toolbar
                if (!$acl->canModerate($row)) {
                    $tmpl->addVar('tpl_category', 'comments-panel-visible', 0);
                } else {
                    $tmpl->addVar('tpl_category', 'comments-panel-visible', 1);
                    $tmpl->addVar('tpl_category', 'button-edit', $acl->canEdit($row));
                    $tmpl->addVar('tpl_category', 'button-delete', $acl->canDelete($row));
                    $tmpl->addVar('tpl_category', 'button-publish', $acl->canPublish($row));
                    $tmpl->addVar('tpl_category', 'button-ip', $acl->canViewIP($row));
                    $tmpl->addVar('tpl_category', 'button-ban', $acl->canBan($row));
                }

                $tmpl->addVar('tpl_category', 'comment-show-vote', $config->getInt('enable_voting'));
                $tmpl->addVar('tpl_category', 'comment-show-email', $acl->canViewEmail($row));
                $tmpl->addVar('tpl_category', 'comment-show-homepage', $acl->canViewHomepage($row));
                $tmpl->addVar('tpl_category', 'comment-show-title', $config->getInt('comment_title'));
                $tmpl->addVar('tpl_category', 'button-vote', $acl->canVote($row));
                $tmpl->addVar('tpl_category', 'button-quote', $acl->canQuote($row));
                $tmpl->addVar('tpl_category', 'button-reply', $acl->canReply($row));
                $tmpl->addVar('tpl_category', 'button-report', $acl->canReport($row));
                $tmpl->addVar('tpl_category', 'avatar', $acl->check('enable_gravatar') && !$row->deleted);

                $tmpl->addObject('tpl_category', 'comment', $row);

                if (isset($row->_number)) {
                    $tmpl->addVar('tpl_category', 'comment-number', $row->_number);
                } else {
                    $tmpl->addVar('tpl_category', 'comment-number', $i);
                ...
                $items[$row->id] = $tmpl->renderTemplate('tpl_category');
                $lim++;
                if($lim == $limitend){break;}
            }
