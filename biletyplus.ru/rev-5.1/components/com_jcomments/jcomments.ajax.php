 	...
 	public static function addComment($values = array())
 	{
 		...
 		if ($acl->canComment()) {
 			...
 			if (($acl->check('floodprotection') == 1) && (JCommentsSecurity::checkFlood($userIP))) {
 				...
 				//by matt1
 			/*} else if (empty($values['position']) && isset($values['position'])) {
 				self::showErrorMessage(JText::_('ERROR_EMPTY_POSITION'), 'position');*/
 			}
 			else if(isset($values['position']) && JCommentsText::strlen($values['position']) > 127){
 				self::showErrorMessage(JText::_('ERROR_POSITION_IS_TOO_LONG'), 'position');
 			...
 			&& (JCommentsText::strlen($values['comment']) < $config->get('comment_minlength'))) {
 				...
 				$comment->position = $user->id ? $user->name : preg_replace("/[\'\"\>\<\(\)\[\]]?+/i", '', $values['position']);
