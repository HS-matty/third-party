...
if(isset($sort)){
	...
	if (!isset($cid) || $cid==0) {
		$cid = 0;    
		if ($t_page_wo_goods==1) $page_type=1; //title page
	}else $page_type=0; //common page with goods




	create_path($cid); // path[] - path to selected category.
						//also we get: categories[cid]=parent,cat_names[cid]=cid_name
...
if(!$row_num)  	eval("\$index = \"".$tpl->get("no_goods")."\";"); 
...
eval("print \"".$tpl->get("main")."\";"); // Parsing and displaying main template.
