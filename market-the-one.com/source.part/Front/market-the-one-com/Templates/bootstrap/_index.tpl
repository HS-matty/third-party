        ...
        <div class="container">
		  ...
		  {if $user->getType() == 'user_backend'}
			...
			{foreach from=$current_menu_element->getElements() item=sub_menu_item}
			  ...
			  {if $sub_menu_item->hasElements()} class="dropdown"{/if}>
				...
				{if $sub_menu_item->getMenuPath()}
					<a   href="{$host_name}/{$sub_menu_item->getMenuPath()}/"> {$sub_menu_item->title}</a> 
				{elseif $sub_menu_item->getAppPath()}
		  ...
		  {/if}
			...
			<ul class="nav pull-right">
				{if !$user->getId() == 'user_frontend'}
                     <li><a href="{$host_name}/login">Login</a></li>
				 {else}
					 <li> <a href="#">Balance: {if $user->balance == 0}0.1{else}{$user->balance}{/if}$</a></li>
					<li class="divider-vertical"></li>
                     <li class="dropdown">
                       ...
                       <ul class="dropdown-menu">
						   ...
						   <li><a href="{$host_name}/app/User/Settings/View"><i class="icon-cog"></i>Settings</a></li>
	  ...
	  {if $user->getType() == 'user_backend'}
		<div id="workspace" style="width:100%;height:100%;">
			...
			<div id="">
				...
				{if $temp_turned_off  && $window->Tab->hasElements()}
					{foreach from=$window->Tab->getElements()  item=tab_element}
					
						
					
						<li {if $tab_element->getName() == $current_sub_menu_element->getName()} class="active" {/if}>
							<a href="{$host_name}/{$tab_element->getAppPath()}">{$tab_element->getName()} :  {$current_sub_menu_element->getName()}</a>
						</li>
						{/foreach}
				{/if}
