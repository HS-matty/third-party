			...
			<thead>
				<tr >
					{foreach from=$grid->row->getFields() item=el}
						{if !$el->getParam('is_disabled')}<th><a href="?post=1&order_by={if $el->table}{$el->table}.{/if}{$el->name}&direction={$next_direction}&{$form_params_string}" style="font-size:12px">{$el->title}<img src="{$host_name}/public/icons/resize-2-(16-16).png"></a></th>{/if}
					{/foreach}
		...
		<ul>
			...
			{else}
				...
				{assign var="index" value=$smarty.section.page.index }
					...
					<li  class="li_nohover" {if $index == $current_page} class="active" {/if}><a href="?page={$index}&post=1&order_by={$order_by}&direction={$direction}&{$form_params_string}">{$index+1}</a></li>
				{/section}
				
			{/if}
			{$current_page}
			<li {if $current_page == ($rowset->count_pages_total - 1)} class="disabled"{/if}><a href="{if $current_page == $rowset->count_pages_total}#{else}?page={$next_page}&post=&order_by={$order_by}&direction={$direction}&{$form_params_string}{/if}">&raquo;</a></li>
