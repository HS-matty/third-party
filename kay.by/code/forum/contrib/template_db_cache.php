...
class Template {
	...
	function pparse($handle)
	{
		if( empty($this->compiled_code[$handle]) )
		{
			...
			$sql = "REPLACE INTO phpbb_template_cache (template_name, template_handle, template_cached, template_compile) VALUES ('" . addslashes($this->files[$handle]) . "', '" . addslashes($handle) . "', " . filemtime($this->files[$handle]) . ", '" . addslashes($this->compiled_code[$handle]) . "')";
			if ( !($result = $this->db->sql_query($sql)) )
			{
	...
	function assign_var_from_handle($varname, $handle)
	{
		if( empty($this->compiled_code[$handle]) )
		{
			...
			$sql = "REPLACE INTO phpbb_template_cache (template_name, template_handle, template_echo, template_cached, template_compile) VALUES ('" . addslashes($this->files[$handle]) . "', '" . addslashes($handle) . "', 0, " . filemtime($this->files[$handle]) . ", '" . addslashes($code) . "')";
			if ( !($result = $this->db->sql_query($sql)) )
			{
				die("Couldn't insert template into cache!");
			}
