...
function index() {
    ...
    $result = sql_query("select cid, title, cdescription from $prefix"._links_categories." order by title", $dbi);
    $count = 0;
    while(list($cid, $title, $cdescription) = sql_fetch_row($result, $dbi)) {
	$cresult = sql_query("select * from $prefix"._links_links." where cid=$cid", $dbi);
	$cnumrows = sql_num_rows($cresult, $dbi);
	echo "<td><font class=\"option\"><strong><big>&middot;</big></strong> <a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewlink&amp;cid=$cid\"><b>$title</b></a> ($cnumrows)</font>";
	...
	$result2 = sql_query("select sid, title from $prefix"._links_subcategories." where cid=$cid order by title limit 0,3", $dbi);
	$space = 0;
	while(list($sid, $stitle) = sql_fetch_row($result2, $dbi)) {
    	    if ($space>0) {
		echo ",&nbsp;";
	    }
	    echo "<font class=\"content\"><a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewslink&amp;sid=$sid\">$stitle</a></font>";
	    $space++;
	}
	if ($count<1) {
	    echo "</td><td>&nbsp;&nbsp;&nbsp;&nbsp;</td>";
    ...
    $result=sql_query("select * from $prefix"._links_subcategories."", $dbi);
...
function AddLink() {
    ...
    if (is_user($user) || $links_anonaddlinklock == 1) {  /* 06-24-01 Bug fix : changed $links_anonaddlinklock != 1 to $links_anonaddlinklock == 1 */
		...
		$result2=sql_query("select sid, title from $prefix"._links_subcategories." where cid=$cid order by title", $dbi);
		while(list($sid, $stitle) = sql_fetch_row($result2, $dbi)) {
...
function NewLinksDate($selectdate) {
	...
	$result3=sql_query("select title from $prefix"._links_subcategories." where sid=$sid", $dbi);
...
function viewlink($cid, $min, $orderby, $show) {
    ...
    echo "<br>";
    OpenTable();
    $result=sql_query("select title from $prefix"._links_categories." where cid=$cid", $dbi);
    list($title) = sql_fetch_row($result, $dbi);
    echo "<center><font class=\"option\"><b>"._CATEGORY.": $title</b></font></center><br>";
    $carrytitle = $title;
    $subresult=sql_query("select sid, title from $prefix"._links_subcategories." where cid=$cid order by title", $dbi);
    $numrows = sql_num_rows($subresult, $dbi);
    if ($numrows != 0) {
	$scount = 0;
	echo "<center><font class=\"content\">"._LALSOAVAILABLE." <i>$title</i> "._SUBCATEGORIES.":</font></center><br>"
    	    ."<table align=\"center\" border=\"0\"><tr>";
    	while(list($sid, $title) = sql_fetch_row($subresult, $dbi)) {
	    $result2 = sql_query("select * from $prefix"._links_links." where sid=$sid", $dbi);
	    $numrows = sql_num_rows($result2, $dbi);
    	    echo "<td><a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewslink&amp;sid=$sid\">$title</a> ($numrows)&nbsp;&nbsp;</td>";
    	    $scount++;
    	    if ($scount==4) {
    	    	echo "</tr><tr>";
    	    	$scount = 0;
    ...
    $result=sql_query("select lid, title, description, date, hits, linkratingsummary, totalvotes, totalcomments from $prefix"._links_links." where cid=$cid AND sid=0 order by $orderby limit $min,$perpage", $dbi);
    $fullcountresult=sql_query("select lid, title, description, date, hits, linkratingsummary, totalvotes, totalcomments from $prefix"._links_links." where cid=$cid AND sid=0", $dbi);
...
function viewslink($sid, $min, $orderby, $show) {
    ...
    $result = sql_query("select cid, title from $prefix"._links_subcategories." where sid=$sid", $dbi);
    list($cid, $stitle) = sql_fetch_row($result, $dbi);

    $result2 = sql_query("select cid, title from $prefix"._links_categories." where cid=$cid", $dbi);
    list($cid, $title) = sql_fetch_row($result2, $dbi);

    echo "<center><font class=\"option\"><b><a href=\"modules.php?name=Web_Links\">"._MAIN."</a> / <a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewlink&amp;cid=$cid\">$title</a> / $stitle</b></font></center>";
    $orderbyTrans = convertorderbytrans($orderby);
    echo "<br><center><font class=\"content\">"._SORTLINKSBY.": "
	.""._TITLE." (<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewslink&amp;sid=$sid&amp;orderby=titleA\">A</a>\<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewslink&amp;sid=$sid&amp;orderby=titleD\">D</a>)"
	." "._DATE." (<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewslink&amp;sid=$sid&amp;orderby=dateA\">A</a>\<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewslink&amp;sid=$sid&amp;orderby=dateD\">D</a>)"
	." "._RATING." (<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewslink&amp;sid=$sid&amp;orderby=ratingA\">A</a>\<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewslink&amp;sid=$sid&amp;orderby=ratingD\">D</a>)"
        ." "._POPULARITY." (<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewslink&amp;sid=$sid&amp;orderby=hitsA\">A</a>\<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewslink&amp;sid=$sid&amp;orderby=hitsD\">D</a>)"
	."<br><b>"._SITESSORTED.": $orderbyTrans</b></font></center><br><br>";
    $result=sql_query("select lid, url, title, description, date, hits, linkratingsummary, totalvotes, totalcomments from $prefix"._links_links." where sid=$sid order by $orderby limit $min,$perpage", $dbi);
    $fullcountresult=sql_query("select lid, title, description, date, hits, linkratingsummary, totalvotes, totalcomments from $prefix"._links_links." where sid=$sid", $dbi);
    $totalselectedlinks = sql_num_rows($fullcountresult, $dbi);
    echo "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"10\" border=\"0\"><tr><td><font class=\"content\">";
    $x=0;
    while(list($lid, $url, $title, $description, $time, $hits, $linkratingsummary, $totalvotes, $totalcomments)=sql_fetch_row($result, $dbi)) {
	$linkratingsummary = number_format($linkratingsummary, $mainvotedecimal);
	$title = stripslashes($title); $description = stripslashes($description);
        echo "<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=visit&amp;lid=$lid\" target=\"new\">$title</a>";
	newlinkgraphic($datetime, $time);
	...
	echo "<br><br>"
        ...
        $prev=$min-$perpage;
        if ($prev>=0) {
    	    echo "&nbsp;&nbsp;<b>[ <a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewslink&amp;sid=$sid&amp;min=$prev&amp;orderby=$orderby&amp;show=$show\">"
    		." &lt;&lt; "._PREVIOUS."</a> ]</b> ";
      	}
        $counter = 1;
        $currentpage = ($max / $perpage);
		...
		echo "<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewslink&amp;sid=$sid&amp;min=$mintemp&amp;orderby=$orderby&amp;show=$show\">$counter</a> ";
	    }
            $counter++;
        }
        $next=$min+$perpage;
        if ($x>=$perpage) {
    	    echo "&nbsp;&nbsp;<b>[ <a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewslink&amp;sid=$sid&amp;min=$max&amp;orderby=$orderby&amp;show=$show\">"
    		." "._NEXT." &gt;&gt;</a> ]</b> ";
        }
    }
    echo "</td></tr></table>";
...
function search($query, $min, $orderby, $show) {
    ...
    $resultx = sql_query("select * from $prefix"._links_subcategories." where title LIKE '%$query%' ORDER BY title DESC", $dbi);
    ...
    if ($query != "") {
		...
		$result2 = sql_query("select cid, sid, title from $prefix"._links_subcategories." where title LIKE '%$query%' ORDER BY title DESC", $dbi);
		...
		while(list($cid, $sid, $stitle) = sql_fetch_row($result2, $dbi)) {
		    $cate = sql_query("select title from $prefix"._links_categories." where cid=$cid", $dbi);
		    list($ctitle) = sql_fetch_row($cate, $dbi);
		    $res = sql_query("select * from $prefix"._links_links." where sid=$sid", $dbi);
		    $numrows = sql_num_rows($res, $dbi);
		    $ctitle = ereg_replace($query, "<b>$query</b>", $ctitle);
		    $stitle = ereg_replace($query, "<b>$query</b>", $stitle);
		    echo "<strong><big>&middot;</big></strong>&nbsp;<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=viewslink&amp;sid=$sid\">$ctitle / $stitle</a> ($numrows)<br>";
		}
	echo "<br><table width=\"100%\" bgcolor=\"$bgcolor2\"><tr><td><font class=\"option\"><b>"._LINKS."</b></font></td></tr></table>";
    	...
    	echo "<br><font class=\"content\">"._SORTLINKSBY.": "
    	    .""._TITLE." (<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=search&amp;query=$query&amp;orderby=titleA\">A</a>\<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=search&amp;query=$query&amp;orderby=titleD\">D</a>)"
    	    .""._DATE." (<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=search&amp;query=$query&amp;orderby=dateA\">A</a>\<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=search&amp;query=$query&amp;orderby=dateD\">D</a>)"
    	    .""._RATING." (<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=search&amp;query=$query&amp;orderby=ratingA\">A</a>\<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=search&amp;query=$query&amp;orderby=ratingD\">D</a>)"
    	    .""._POPULARITY." (<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=search&amp;query=$query&amp;orderby=hitsA\">A</a>\<a href=\"modules.php?op=modload&name=Web_Links&file=index&l_op=search&amp;query=$query&amp;orderby=hitsD\">D</a>)"
	    ...
	    $result4 = sql_query("select title from $prefix"._links_subcategories." where sid=$sid", $dbi);
...
function viewlinkcomments($lid, $ttitle) {
	...
	$useravgrating = $useravgrating + $rating2;
    	...
    	echo "<tr><td bgcolor=\"$bgcolor2\">"
    	    ."<font class=\"content\"><b> "._USER.": </b><a href=\"$nukeurl/user.php?op=userinfo&amp;uname=$ratinguser\">$ratinguser</a></font>"
	    ."</td>"
	    ."<td bgcolor=\"$bgcolor2\">"
...
function brokenlinkS($lid, $modifysubmitter) {
    ...
    sql_query("insert into $prefix"._links_modrequest." values (NULL, $lid, 0, 0, '', '', '', '$ratinguser', 1)", $dbi);
    include("header.php");
    menu(1);
    echo "<br>";
    OpenTable();
...
function modifylinkrequest($lid) {
    ...
    if ($blocknow != 1) {
		...
		$result3=sql_query("select sid, title from $prefix"._links_subcategories." where cid=$ccid order by title", $dbi);
