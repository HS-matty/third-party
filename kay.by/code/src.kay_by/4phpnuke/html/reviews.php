...
function write_review() {
    global $admin, $sitename, $user, $cookie, $prefix, $currentlang, $multilingual, $dbi;
    include ('header.php');
    OpenTable();
    echo "
...
function send_review($date, $title, $text, $reviewer, $email, $score, $cover, $url, $url_title, $hits, $id, $rlanguage) {
	...
	sql_query("INSERT INTO $prefix"._reviews."_add VALUES (NULL, '$date', '$title', '$text', '$reviewer', '$email', '$score', '$url', '$url_title', '$rlanguage')", $dbi);
...
function reviews_index() {
    ...
    $result = sql_query("select title, description from $prefix"._reviews."_main", $dbi);
...
function savecomment($xanonpost, $uname, $id, $score, $comments) {
    ...
    sql_query("insert into $prefix"._reviews."_comments values (NULL, '$id', '$uname', now(), '$comments', '$score')", $dbi);
    Header("Location: reviews.php?op=showcontent&id=$id");
}

function r_comments($id, $title) {
    global $admin, $prefix, $dbi;
    $result = sql_query("select cid, userid, date, comments, score from $prefix"._reviews."_comments where rid='$id' ORDER BY date DESC", $dbi);
...
function del_review($id_del) {
	...
	sql_query("delete from $prefix"._reviews."_comments where rid='$id_del'", $dbi);
...
function del_comment($cid, $id) {
    global $admin, $prefix, $dbi;
    if (is_admin($admin)) {
        sql_query("delete from $prefix"._reviews."_comments where cid='$cid'", $dbi);
        Header("Location: reviews.php?op=showcontent&id=$id");
    } else {
        echo "ACCESS DENIED";
    }
