...
// Add a New Sub-Category
	...
	while(list($ccid, $ctitle) = sql_fetch_row($result, $dbi)) {
...
// Add a New Link to Database
	...
	OpenTable();
	echo "<form method=\"post\" action=\"admin.php\">"
	    ."<font class=\"option\"><b>"._ADDNEWLINK."</b><br><br>"
...
function LinksModLink($lid) {
    ...
    GraphicAdmin($hlpfile);
    global $anonymous;
    $result = sql_query("select cid, sid, title, url, description, name, email, hits from $prefix"._links_links." where lid=$lid", $dbi);
    OpenTable();
    ...
    while(list($cid, $sid, $title, $url, $description, $name, $email, $hits) = sql_fetch_row($result, $dbi)) {
...
function LinksDelCat($cid, $sid, $sub, $ok=0) {
    ...
    if($ok==1) {
	if ($sub>0) {
    	    sql_query("delete from $prefix"._links_subcategories." where sid=$sid", $dbi);
	    sql_query("delete from $prefix"._links_links." where sid=$sid", $dbi);
	} else {
	    sql_query("delete from $prefix"._links_categories." where cid=$cid", $dbi);
	    sql_query("delete from $prefix"._links_subcategories." where cid=$cid", $dbi);
	    sql_query("delete from $prefix"._links_links." where cid=$cid AND sid=0", $dbi);
	}
	Header("Location: admin.php?op=links");    
    } else {
	include("header.php");
...
function LinksAddCat($title, $cdescription) {
	...
	sql_query("insert into $prefix"._links_categories." values (NULL, '$title', '$cdescription')", $dbi);
	Header("Location: admin.php?op=links");
    }
}
...
function LinksAddSubCat($cid, $title) {
    global $prefix, $dbi;
    $result = sql_query("select cid from $prefix"._links_subcategories." where title='$title' AND cid='$cid'", $dbi);
    $numrows = sql_num_rows($result, $dbi);
    if ($numrows>0) {
	include("header.php");
	GraphicAdmin($hlpfile);
	...
	sql_query("insert into $prefix"._links_subcategories." values (NULL, '$cid', '$title')", $dbi);
	Header("Location: admin.php?op=links");
    }
}
