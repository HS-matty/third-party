...
function DownloadsDelCat($cid, $sid, $sub, $ok=0) {
    ...
    if($ok==1) {
	if ($sub>0) {
    	    sql_query("delete from $prefix"._downloads_subcategories." where sid=$sid", $dbi);
	    sql_query("delete from $prefix"._downloads_downloads." where sid=$sid", $dbi);
	} else {
	    sql_query("delete from $prefix"._downloads_categories." where cid=$cid", $dbi);
	    sql_query("delete from $prefix"._downloads_subcategories." where cid=$cid", $dbi);
	    sql_query("delete from $prefix"._downloads_downloads." where cid=$cid AND sid=0", $dbi);
	}
	Header("Location: admin.php?op=downloads");    
    } else {
	include("header.php");
...
function DownloadsAddCat($title, $cdescription) {
	...
	sql_query("insert into $prefix"._downloads_categories." values (NULL, '$title', '$cdescription')", $dbi);
	Header("Location: admin.php?op=downloads");
    }
}
...
function DownloadsAddSubCat($cid, $title) {
    global $prefix, $dbi;
    $result = sql_query("select cid from $prefix"._downloads_subcategories." where title='$title' AND cid='$cid'", $dbi);
    $numrows = sql_num_rows($result, $dbi);
    if ($numrows>0) {
	include("header.php");
	GraphicAdmin($hlpfile);
	...
	sql_query("insert into $prefix"._downloads_subcategories." values (NULL, '$cid', '$title')", $dbi);
	Header("Location: admin.php?op=downloads");
    }
}
