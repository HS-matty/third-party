...
<body>
	...
	<div id="wrapper" align="center">
		...
		<form action="{$HostName}/auth/login/" method="post">
			...
			<div id="exit"><a href="{$HostName}/auth/logout/">Выйти</a></div>
			{else}
				Авторизация 
				...
				<a href="{$HostName}/auth/register/">Регистрация</a>&nbsp;&nbsp;&nbsp;&nbsp;
				{/if}
			</div>
			</form>
			<div id="logo">
		...
		{if $search}
			...
			{include file=$View->getIndexTmpl('calculator/form.tpl')}
		</div>	
		</div>
			...
			{if  !$User->isLogined('RegisteredUser') || true}
			<div style="margin-bottom: 30px;">
			{foreach from=$services item=s}
			<a href="{$HostName}/service/{$s.alias}/"><img src="{$HostName}/images/ico.jpg" border="0" alt="{$s.short_description}"></a>
			{/foreach}
		</div>{/if}
...
{if $User->Searches->getItems('default')}
<h4><a href="{$HostName}/user/cart/">Корзина</h4>
<ul>
{foreach from=$User->Searches->getItems('default',1) item=c}
<li><a href="{$HostName}/go/item/?listing_id={$c.listing_id}">{$c.short_description}</a></li>
{/foreach}
</ul>
<br /><br /><br />
{/if}</div>
		...
		<div id="main_content">
			<div id="kredit_text">
			{if $path}
			<div align="right"><a href="{$HostName}/">Главная</a>
			{foreach from=$path item=p}
			 &raquo;	<a href="{$HostName}/content/node/{$p.category_id}/">{$p.short_description}</a> 
			{/foreach}
			{if $Record && $RecordAction }
				&raquo; <a href="{$HostName}/content/record/{$Record->Data.listing_id}/">{$Record->Data.short_description}</a> 
			{/if}
			</div>
			{/if}
			{if $Record && !$UsePath}
		...
		<div id="bottom">
			...
			<div id="meshki">
			<img src="{$HostName}/images/meshki.gif" />
			</div>
		</div>
	</div>
</body>
