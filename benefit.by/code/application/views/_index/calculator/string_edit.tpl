...
<td>
{$f->View->ViewObject->Create()}</td></tr>

{else}
<tr>
<td align="left"><div class="search_text">{$f->Title}{if $f->isRequired}<font color="red">*</font>{/if}:</div>
	...
	{else}
		...
		{else}
			...
			{else}
			<input name="{$f->ID}"  id="sum" {if $f->View->Type == 'password'} type="password" {else} type="text"  value="{$f->getValue()}" {/if}>
			{/if}
		{/if}
