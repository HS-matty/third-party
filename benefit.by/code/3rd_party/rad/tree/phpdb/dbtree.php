...
//              Baturov Michail(docker@inbox.ru)
...
class CDBTree {
	...
	var $TitleField = 'short_description';
	var $qryParams = '';
	var $qryFields = '';
	var $qryTables = '';
	var $qryWhere = '';
	...
	var $Params=''; //added by descent 17.11.2006
	...
	//************************************************************************
	// Author : Baturov Michail(docker@inbox.ru)
	// Gets a parent info
	// $child_id : an ID of the child element
	function getParentInfo($child_id) {
		$res = $this->db->fetch_array($this->getParent($child_id));
		return $this->getNodeInfo($res[$this->id]);
	}

	//************************************************************************
	// Author : Baturov Michail(docker@inbox.ru)
	// Finds 2 nodes, which will be swaped during the branch shift
	// $ID : an ID of the element
	// $order : shift order ('up' or 'down')
	function getReplacePairNodes($ID, $order) {
		list($originLeft, $originRight, $originLevel) = $this->getNodeInfo($ID);
		list($parentLeft, $parentRight, $parentLevel) = $this->getParentInfo($ID);


		if ($order == 'down') {
			$res = $this->db->sql2array('SELECT `'.$this->id.'` FROM `'.$this->table.'`
                                          WHERE `'.$this->level.'` = "'.$originLevel.'" AND 
                                                `'.$this->left.'`  > "'.$originLeft.'"  AND 
                                                `'.$this->left.'`  > "'.$parentLeft.'"  AND 
	...
	//************************************************************************
	// Author : Baturov Michail(docker@inbox.ru)
	// Shifts a branch up or down within one level
	// $ID : an ID of the element
	// $order : shift order ('up' or 'down')
	function changeBranchOrder($ID, $order) {
		if ((int)$ID <= 1) return false;

		// ������� ������� � ������, ������� ����� ������ �������.
		// ����� ������� ��� ������ ��������� ����, ���� ���������� ����� �������� ����
		// � ����� ������� ���� ��������� ��� �������� ��� �����.
		if (!($pair = $this->getReplacePairNodes((int)$ID, $order))) return false;


		$tmpId = $this->insertNear($pair[1], array('title' => 'tmpnode'));
	...
	//************************************************************************
	// Checks: if category table is an empty
	// $ID : an ID of the element
	function isEmpty() {
		$dbr = $this->db->query('SELECT * FROM '.$this->table.'');
		return (!$this->db->num_rows($dbr));
	}
...
class CDBTreeExt extends CDBTree  {
	...
	function enumChildren($ID, $start_level=1, $end_level=1,$LimitRows = null,$OrderBy = null,array $_params  = null) {
		if($start_level < 0) trigger_error("phpDbTree error: ".$this->db->error(), E_USER_ERROR);
		...
		//add Params
		$Params = "$this->table.*, ";// $this->table.'.title,';
		$this->sql = $this->sqlComposeSelect(array(
		$Params, // Params
		'', // Fields
		$this->table.' _'.$this->table.', '.$this->table, // Tables
		...
		if($_params['is_active']){
			...
			$this->sql .= " AND {$this->table}.is_active = 1 ";
		}
		if($OrderBy){
			$this->sql .= 'ORDER BY '.$this->table.'.'.$OrderBy.' desc';
		}else {
			$this->sql .= 'ORDER BY '.$this->table.'.'.$this->left.' asc';

		}
		if($LimitRows){
			$this->sql .= " LIMIT $LimitRows";
		}
		...
		if($Db->DoNotUseListQuery){
			$Db->query($this->sql);
			return $Db->fetch_all_assoc();

		}else {

			$Db->performSelectQueryForList($this->sql);
	...
	function &getNodeInfoExt($ID) {
		...
		if($Db->rows() && $Data =& $Db->fetch_assoc()){

			//by descent
			return $Data;
		}
		$Arr =  array((int)$Data[$this->left], (int)$Data[$this->right], (int)$Data[$this->level]);
		return $Arr;
	...
	function enumPathExt($ID, $showRoot=false) {
	...
	function ifCategoryExists($Cid){
		global $Db;
		$Db->query("SELECT $this->id FROM $this->table WHERE $this->id = $Cid");
	...
	public function setParams($Param){
		/*	if($this->Params) $this->Params .= ', '.$this->table.'.'.$Param;
		else $this->Params = $this->table.'.'.$Param;*/
	}
	...
	function &getTreeArray(){
		...
		global $Db;
		$sql="SELECT * FROM ".$this->table." WHERE $this->id != 1 ORDER BY ".$this->left." ASC";
	...
	function &getConvertedTreeArray($AsArray = false){

		global $Db;
		$sql="SELECT * FROM ".$this->table." WHERE $this->id != 1 ORDER BY ".$this->left." ASC";
		//$sql = "SELECT  IF ( A.$this->level = 0, B.$this->left, A.$this->left ) AS cat, A.clevel,  B.cid FROM ".$this->table." A INNER  JOIN ".$this->table." B ON ( ( B.clevel = A.clevel + 1 ) AND ( B.cleft = B.cright - 1 ) AND B.cleft BETWEEN A.cleft AND A.cright )  WHERE ( A.cleft != A.cright - 1) ORDER BY cat, B.cid";
		...
		$Data =& $this->transformArrayToTree($Arr);
	...
	function &transformArrayToTree($Arr,$AsTree = true){
		...
		foreach ($Arr as &$f)
		{
			...
			if($f[$this->level]>$i_level)
			{
				//���� �� ������ �� ������� ����, ��� ���� ������, ������:
				// - ����� ����������� ������ ����� �� 1 ������ ��������
				// - ��� ���������� ���� �� ���� ������� �� ���������� ������
				//if($f[$this->level]!=$i_level+1 || !count($a_path[$i_level]))
				//return false; //������ ���� �� ������ - ������ � ������


				$a_path[$f[$this->level]]=&$a_path[$i_level][count($a_path[$i_level])-1]['a_tree'];


				$ParentId = (int) @$a_path[$i_level][count($a_path[$i_level])-1]['k_item'];
			}else {


				$LevelDiff  = $i_level - $f[$this->level];
				if($LevelDiff){
					$ParentLevel = $f[$this->level ]-$LevelDiff;
					$LastElement = count(@$a_path[$ParentLevel])-1;
					$ParentId = (int) @$a_path[$ParentLevel][$LastElement]['k_item'];
				}
			...
			$A = array(
			'k_item' => $f[$this->id],
			's_name' => $f[$this->TitleField],
			'parent_id'=>$ParentId,
			'a_tree' => array()
			);
			$a_path[$f[$this->level]][] = $A;
			$NewArr[] = $A;

			$i_level=$f[$this->level];
		...
		if($AsTree) return $a_tree;

		return $NewArr;
	}


	//original function with addon
	function delete($ID) {
